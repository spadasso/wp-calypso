...
let make ::name ::count ::onClick _children => {
  let clickIt _event _state _self => {
  	onClick ();
  };
  {
	  ...component,
	  render: fun () self =>
		<div>
			...
			<button _type="button" className="button is-compact" onClick=(self.update clickIt)>(text (" -> " ^ count))</button>
		</div>
  }
};
...
let counter state::(state: (undefined int)) action => {
	let prev = switch (Undefined.to_opt state) {
		| Some s => s
		| None => 0
	};
	switch action##_type {
