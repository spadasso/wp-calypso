...
import TableHeading from 'woocommerce/components/table/table-heading';
	...
	renderOrderItems = ( order, i ) => {
		...
		return (
			<TableRow key={ i }>
				...
				<TableHeading className="orders__table-name" scope="row">
	...
	render() {
		...
		const headers = (
			<TableRow>
				...
				<TableHeading>{ translate( 'Order' ) }</TableHeading>
				<TableHeading>{ translate( 'Date' ) }</TableHeading>
				<TableHeading>{ translate( 'Fulfillment Status' ) }</TableHeading>
				<TableHeading>{ translate( 'Total' ) }</TableHeading>
			</TableRow>
		);
