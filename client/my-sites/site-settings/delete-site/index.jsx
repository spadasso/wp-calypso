...
export const DeleteSite = React.createClass( {
	...
	render: function() {
		const site = this.state.site,
			...
			exportLink = config.isEnabled( 'manage/export' )
		...
		const deleteButtons = [
			<Button onClick={ this.closeConfirmDialog }>{ this.translate( 'Cancel' ) }</Button>,
							...
							>
								...
								<g transform="translate(0 -5)" fill="none" fillRule="evenodd">
									...
									<text fontFamily="Open Sans" fontSize="11" fontWeight="526" fill="#FFF">
										<tspan x="69.736" y="172">.ZIP</tspan>
									</text>
									<path
				...
				<ActionPanel>
					...
					>
						...
						<p className="delete-site__confirm-paragraph">
							{ this.translate(
								'Please type in {{warn}}%(siteAddress)s{{/warn}} in the field below to confirm. Your site will then be gone forever.',
								{
									components: {
		...
		SiteListActions.deleteSite(
			...
			function( error ) {
				if ( ! error ) {
					...
					error.message = this.translate(
						'You must cancel any active subscriptions prior to deleting your site.',
					);
