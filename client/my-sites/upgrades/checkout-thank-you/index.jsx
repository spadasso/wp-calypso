...
const CheckoutThankYou = React.createClass( {
	...
	redirectIfThemePurchased() {
		...
		if (
			this.props.receipt.hasLoadedFromServer && purchases.length > 0 && purchases.every( isTheme )
		) {
	...
	goBack() {
		if ( this.isDataLoaded() && ! this.isGenericReceipt() ) {
			...
			if ( purchases.some( isPlan ) ) {
				...
				return page(
					upgradesPaths.domainManagementEmail( this.props.selectedSite.slug, purchase.meta ),
				);
			}
		}
