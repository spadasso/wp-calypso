...
class DomainDetailsForm extends PureComponent {
	...
	getAllFieldValues() {
		...
		allFieldValues.phone = toIcannFormat(
			allFieldValues.phone,
			countries[ this.state.phoneCountryCode ],
		);
	...
	needsFax() {
		return (
			formState.getFieldValue( this.state.form, 'countryCode' ) === 'NL' &&
			cartItems.hasTld( this.props.cart, 'nl' )
		);
	}
	...
	renderExtraDetailsForm() {
		return (
			<ExtraInfoFrForm
				isProbablyOrganization={ Boolean(
					formState.getFieldValue( this.state.form, 'organization' ),
				) }
	...
	focusFirstError() {
		this.refs[
			kebabCase( head( map( formState.getInvalidFields( this.state.form ), 'name' ) ) )
		 ].focus();
	}
