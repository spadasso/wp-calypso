...
export default React.createClass( {
	...
	renewLink( count ) {
		const fullMessage = this.translate( 'Renew it now.', 'Renew them now.', {
			count,
	...
	newDomains() {
		...
		const newDomains = this.getDomains().filter(
			...
			hasNewPrimaryDomain = newDomains.some(
	...
	unverifiedDomainsCanManage() {
		...
		if ( domains.length === 1 ) {
			...
			if ( severity === 'is-error' ) {
				...
				compactMessage = this.translate( 'Issues with {{strong}}%(domain)s{{/strong}}.', {
					components: { strong: <strong /> },
					args: { domain },
				} );
				...
				compactMessage = this.translate( 'Please verify {{strong}}%(domain)s{{/strong}}.', {
					components: { strong: <strong /> },
					args: { domain },
				} );
			}
				...
				<span>
					...
					<ul>
						{ domains.map(
							( { name } ) => (
								<li key={ name }>
									{ name } <a href={ editLink( name ) }>{ this.translate( 'Fix' ) }</a>
								</li>
							),
						) }
					</ul>
