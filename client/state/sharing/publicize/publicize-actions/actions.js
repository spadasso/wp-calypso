	...
	return dispatch => {
		...
		return wpcom.req.get(
			...
			( error, data ) => {
				if ( error || ! data.items ) {
					return dispatch(
						{ type: PUBLICIZE_SHARE_ACTIONS_SCHEDULED_REQUEST_FAILURE, siteId, postId, error },
					);
				}
				...
				dispatch(
					{ type: PUBLICIZE_SHARE_ACTIONS_SCHEDULED_REQUEST_SUCCESS, siteId, postId, actions },
				);
			},
		);
	};
...
export function fetchPostShareActionsPublished( siteId, postId ) {
	return dispatch => {
		...
		return wpcom.req.get( getPublishedPath, ( error, data ) => {
			if ( error || ! data.items ) {
				return dispatch(
					{ type: PUBLICIZE_SHARE_ACTIONS_PUBLISHED_REQUEST_FAILURE, siteId, postId, error },
				);
			}
			...
			data.items.forEach( action => actions[ action.ID ] = action );
			dispatch(
				{ type: PUBLICIZE_SHARE_ACTIONS_PUBLISHED_REQUEST_SUCCESS, siteId, postId, actions },
			);
		} );
	};
}
	...
	return dispatch => {
		...
		return wpcom.req.get(
			...
			( error, data ) => {
				if ( error || ! data ) {
					return dispatch(
						{ type: PUBLICIZE_SHARE_ACTION_DELETE_FAILURE, siteId, postId, actionId, error },
					);
				}
		...
		return wpcom.req.put(
			...
			( error, data ) => {
				if ( error || ! data.item ) {
					...
					return dispatch(
						{ type: PUBLICIZE_SHARE_ACTION_EDIT_FAILURE, siteId, postId, actionId, error },
					);
				}
				...
				dispatch(
					{ type: PUBLICIZE_SHARE_ACTION_EDIT_SUCCESS, siteId, postId, actionId, item: data.item },
				);
			},
		);
	};
